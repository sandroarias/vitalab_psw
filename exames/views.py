...
def fechar_pedido(request):
    exames_id = request.POST.getlist('exames')
    soliciatacao_exames = TiposExames.objects.filter(id__in=exames_id)
    ...
    for exame in soliciatacao_exames:
        soliciatacao_exames_temp = SolicitacaoExame(
            usuario=request.user,
            exame=exame,
            status="E"
        )
        soliciatacao_exames_temp.save()
        pedido_exame.exames.add(soliciatacao_exames_temp)
...
def cancelar_pedido(request, pedido_id):
    ...
    if not pedido.usuario == request.user:
        messages.add_message(request, constants.ERROR, 'Esse pedido não e seu!')
        return redirect('/exames/gerenciar_pedidos/')
...
@login_required
def perimitir_abrir_exame(request, exame_id):
    exame = SolicitacaoExame.objects.get(id=exame_id)
    if not exame.requer_senha:
        # TUDO: verificar se tem o PDF do resultado
...
def acesso_medico(request, token):
    ...
    pedidos = PedidosExames.objects.filter(usuario=acesso_medico.usuario).filter(data__gte=acesso_medico.data_exames_iniciais).filter(data__lte=acesso_medico.data_exames_iniciais)
